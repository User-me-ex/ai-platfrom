...
// Voice gateway: serves ws://<host>:<port>/voice and bridges to Gemini Live
// using the API key of the active `gemini`/`vertex` provider connection.
...
http.createServer = (...args) => {
  ...
  server.emit = function (event, ...eventArgs) {
    ...
    if (event === "upgrade" && req && String(req.headers.upgrade || "").toLowerCase() === "websocket") {
      const urlPath = String(req.url || "").split("?")[0];
      if (urlPath === "/voice") {
        voiceGateway.handleUpgrade(req, socket, head);
        return true;
      }
    }
